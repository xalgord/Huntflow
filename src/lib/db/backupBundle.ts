...
export async function collectBackupPayload(): Promise<BackupPayload> {
  ...
  for (const blob of evidenceBlobs) {
    // EvidenceBlobDB normalizes to the canonical `blob` field on read,
    // so we don't have to handle legacy aliases here. Empty blobs still
    // get a row written so the asset id is preserved on restore.
    serializedBlobs.push({
      assetId: blob.assetId,
      size: blob.size,
      mimeType: blob.mimeType,
      updatedAt: blob.updatedAt,
      data: blob.blob ? await blobToBase64(blob.blob) : ''
    });
  }
...
export async function restoreBackupPayload(rawPayload: unknown): Promise<RestoreResult> {
  ...
  await safeRun('evidenceBlobs', async () => {
    ...
    const restored = serialized.map((b) => {
      ...
      // Write the canonical shape only. The `data` legacy alias was
      // retired in the schema; EvidenceBlobDB.put would strip it
      // anyway, but emitting it here would confuse a reader of the
      // restore log.
      return {
        assetId: b.assetId,
        size: b.size,
