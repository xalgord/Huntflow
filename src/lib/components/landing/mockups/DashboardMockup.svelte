    ...
    {#each ['Dashboard', 'Timer', 'Targets', 'Notes', 'Evidence', 'Stats', 'Income'] as item, i}
      <div
        class="flex items-center gap-2 rounded-md px-2 py-1.5 text-[11px] {i === 0
          ...
          : 'text-slate-500'}"
      >
        <span class="h-1 w-1 rounded-full {i === 0 ? 'bg-primary-400' : 'bg-slate-700'}"></span>
        {item}
      </div>
    {/each}
  </aside>
  ...
  <div class="overflow-hidden p-5">
    <header class="mb-4 flex items-center justify-between">
      <div>
        ...
        <h1 class="mt-0.5 text-lg font-semibold text-slate-100">Tuesday, hunt no. 47</h1>
      </div>
    ...
    <div class="grid grid-cols-3 gap-3">
      <div class="rounded-lg border border-slate-800 bg-slate-900 p-3">
        <div class="flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-slate-500">
          <Timer size={11} aria-hidden="true" /> Focused
        </div>
        <p class="mt-1.5 text-2xl font-semibold text-slate-100">47<span class="text-base text-slate-500">h</span></p>
      ...
      <div class="rounded-lg border border-slate-800 bg-slate-900 p-3">
        ...
        <p class="mt-1.5 text-2xl font-semibold text-slate-100">$3,250</p>
      ...
      <div class="rounded-lg border border-slate-800 bg-slate-900 p-3">
        ...
        <p class="mt-1.5 text-2xl font-semibold text-slate-100">8<span class="text-base text-slate-500"> targets</span></p>
        <p class="text-[10px] text-slate-500">3 high priority</p>
      </div>
    </div>

    <!-- Today's hunt panel -->
    <div class="mt-4 rounded-lg border border-slate-800 bg-slate-900 p-3.5">
      <div class="flex items-center justify-between">
        <p class="text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-500">Today&apos;s hunt</p>
        <span class="rounded bg-slate-800 px-2 py-0.5 font-mono text-[10px] text-slate-400">paused</span>
      </div>
      <div class="mt-3 flex items-center gap-4">
        <div class="font-mono text-3xl font-semibold tabular-nums text-primary-300">18:42</div>
        <div class="flex-1">
          <p class="text-[11px] text-slate-400">Active target</p>
          <p class="text-sm font-medium text-slate-100">Tesla Bug Bounty</p>
          <div class="mt-1.5 flex gap-1">
            <span class="rounded bg-slate-800 px-1.5 py-0.5 text-[9px] text-slate-300">recon</span>
            <span class="rounded bg-slate-800 px-1.5 py-0.5 text-[9px] text-slate-300">ssrf</span>
          </div>
        </div>
        <div class="flex gap-2">
          <button class="h-8 w-8 rounded-md bg-primary-600 text-[10px] font-semibold text-slate-950">▶</button>
        </div>
      </div>
    </div>
    ...
    <div class="mt-4 rounded-lg border border-slate-800 bg-slate-900 p-3.5">
      <div class="mb-2 flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-slate-500">
      ...
      {#each [{ name: 'SSRF in image proxy', target: 'Tesla', state: 'Triaged', tone: 'amber' }, { name: 'IDOR on /api/users', target: 'GitLab', state: 'Resolved', tone: 'emerald' }, { name: 'Stored XSS in profile', target: 'Shopify', state: 'New', tone: 'sky' }] as row}
        <div class="flex items-center justify-between border-t border-slate-800/60 py-1.5 first:border-0 first:pt-0">
          <div>
            <p class="text-[11px] font-medium text-slate-100">{row.name}</p>
            <p class="text-[10px] text-slate-500">{row.target}</p>
          </div>
          <span
