<script lang="ts">
  ...
  const vulns = [
    ...
    { name: 'Other', pct: 6, color: 'bg-slate-500' }
  ];
</script>
    ...
    {#each ['Dashboard', 'Timer', 'Targets', 'Notes', 'Evidence', 'Stats', 'Income'] as item, i}
      <div
        class="flex items-center gap-2 rounded-md px-2 py-1.5 text-[11px] {i === 5
          ...
          : 'text-slate-500'}"
      >
        <span class="h-1 w-1 rounded-full {i === 5 ? 'bg-primary-400' : 'bg-slate-700'}"></span>
        {item}
      </div>
    {/each}
  </aside>
  ...
  <div class="overflow-hidden p-5">
    ...
    <div class="grid grid-cols-4 gap-2.5">
      {#each [{ k: 'Total focused', v: '246h', sub: 'all-time' }, { k: 'Best streak', v: '23d', sub: 'this year' }, { k: 'Earned', v: '$18,450', sub: 'lifetime' }, { k: 'Acceptance', v: '67%', sub: 'last 90d' }] as m}
        <div class="rounded-lg border border-slate-800 bg-slate-900 p-2.5">
          <p class="text-[9px] uppercase tracking-wider text-slate-500">{m.k}</p>
          <p class="mt-1 text-lg font-semibold text-slate-100">{m.v}</p>
          <p class="text-[9px] text-slate-500">{m.sub}</p>
        </div>
      {/each}
    </div>
    ...
    <div class="mt-3 rounded-lg border border-slate-800 bg-slate-900 p-3">
      <div class="flex items-center justify-between">
        <p class="text-[10px] font-semibold uppercase tracking-wider text-slate-500">Daily focus</p>
        <p class="text-[9px] text-slate-500">Last 30 days</p>
      </div>
    ...
    <div class="mt-3 grid grid-cols-[1fr_180px] gap-3">
      ...
      <div class="rounded-lg border border-slate-800 bg-slate-900 p-3">
        <p class="mb-2 text-[10px] font-semibold uppercase tracking-wider text-slate-500">Activity heatmap</p>
        ...
        <div class="mt-2 flex items-center justify-end gap-1 text-[9px] text-slate-500">
          <span>less</span>
      ...
      <!-- Vuln breakdown -->
      <div class="rounded-lg border border-slate-800 bg-slate-900 p-3">
        <p class="mb-2 text-[10px] font-semibold uppercase tracking-wider text-slate-500">Vulnerability mix</p>
        <div class="space-y-1.5">
          {#each vulns as v}
            <div>
              <div class="flex justify-between text-[9px]">
                <span class="text-slate-300">{v.name}</span>
                <span class="font-mono text-slate-500">{v.pct}%</span>
              </div>
              <div class="mt-0.5 h-1 overflow-hidden rounded-full bg-slate-800">
