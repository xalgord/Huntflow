...
<div class="grid h-full grid-cols-[180px_220px_1fr] bg-slate-950 text-slate-200">
    ...
    {#each ['Dashboard', 'Timer', 'Targets', 'Notes', 'Evidence', 'Stats', 'Income'] as item, i}
      <div
        class="flex items-center gap-2 rounded-md px-2 py-1.5 text-[11px] {i === 3
          ...
          : 'text-slate-500'}"
      >
        <span class="h-1 w-1 rounded-full {i === 3 ? 'bg-primary-400' : 'bg-slate-700'}"></span>
        {item}
      </div>
    {/each}
  </aside>

  <div class="overflow-hidden border-r border-slate-900 bg-slate-950 p-3">
    <p class="mb-2 text-[10px] font-semibold uppercase tracking-wider text-slate-500">Vault &middot; 24 notes</p>
    {#each notes as n}
      <div
      ...
      >
        <div class="flex items-center gap-1.5">
          <FileText size={10} class={n.active ? 'text-primary-300' : 'text-slate-500'} aria-hidden="true" />
          <p class="truncate text-[11px] {n.active ? 'text-slate-100' : 'text-slate-300'}">{n.title}</p>
        </div>
        <p class="mt-0.5 truncate pl-3.5 text-[9px] text-slate-500">{n.target}</p>
      </div>
    {/each}
  </div>
  ...
  <div class="overflow-hidden p-5">
    <div class="flex items-center gap-2">
      <h2 class="text-base font-semibold text-slate-100">SSRF in image proxy</h2>
    ...
    <div class="mt-1.5 flex items-center gap-1 text-[10px] text-slate-500">
      <Hash size={10} aria-hidden="true" />
      <span>tesla &middot; ssrf &middot; recon &middot; image-proxy</span>
    </div>

    <div class="mt-4 space-y-2.5 text-[11px] leading-5 text-slate-300">
      <p class="text-xs font-semibold text-slate-100">Steps to reproduce</p>
      <ol class="ml-3 list-decimal space-y-0.5 text-[11px] text-slate-300">
        <li>Authenticate as a regular user.</li>
        <li>POST to <span class="rounded bg-slate-800 px-1 font-mono text-[10px] text-emerald-300">/api/image-proxy</span> with a crafted URL.</li>
        <li>Observe the server fetches arbitrary internal endpoints.</li>
      </ol>

      <pre class="mt-2 overflow-hidden rounded-md border border-slate-800 bg-slate-950 p-2.5 font-mono text-[10px] leading-4 text-slate-300"><span class="text-emerald-300">curl</span> -X POST https://target.tld/api/image-proxy \
  ...
  -d <span class="text-amber-300">'&#123;"url":"http://169.254.169.254/latest/meta-data/"&#125;'</span></pre>
      ...
      <p class="text-xs font-semibold text-slate-100">Impact</p>
